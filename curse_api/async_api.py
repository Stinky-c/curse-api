...
class AsyncCurseAPI:
    ...
    def __init__(
        ...
        factory: Type[AsyncAPIFactory] = AsyncAPIFactory,
        **settings,
    ) -> None:
        self._api: AsyncAPIFactory = factory(
    ...
    async def modloader_versions(self) -> List[MinecraftModLoaderIndex]:
        ...
        """
        res = await self._api._get("/v1/minecraft/modloader", params={"includeAll": True})
        res.raise_for_status()
    ...
    async def get_fingerprints(self, fingerprints: List[int]):
        ...
        res = await self._api._post("/v1/fingerprints", params={"fingerprints": fingerprints})
        res.raise_for_status()
